...
from game_data.dataimport.constants import PLS_HEADERS, USER_AGENTS, POSITIONS, MATCHES_DIR
from game_data.dataimport.utils import (WikipediaPlayer, timeout_handler, gen_date, feets_to_meters, timeout,
...
class BRefMatch:
    ...
    def is_crawled(self):
        ...
        """
        return '{0}.json'.format(self.code) in os.listdir('{0}/{1}/{2}/{3}'.format(
                                                          MATCHES_DIR, self.country, self.league, self.season))
    ...
    def _write_match(self):
        filename = '{0}/{1}/{2}/{3}/{4}.json'.format(MATCHES_DIR, self.country, self.league, self.season, self.code)
